...
func TestPrivateInheritedInput(t *testing.T) {
	candidate := syntheticCandidate(t)
	for _, flow := range []string{"broker", "manual"} {
		for _, mode := range []string{"success", "delayed-eof", "deadline", "cancel"} {
			t.Run(flow+"/"+mode, func(t *testing.T) {
				...
				defer cancel()
				command := exec.CommandContext(ctx, os.Args[0], "-test.run=^TestPrivateInheritedInputChild$")
				...
				if mode == "success" || mode == "delayed-eof" {
					...
					if flow == "broker" {
						payload, err = json.Marshal(brokerInput{PEM: string(candidate.PEM)})
						if err != nil {
							t.Fatal("synthetic payload setup failed")
						}
					}
...
func TestPrivateInheritedInputChild(t *testing.T) {
	...
	if fixture[0] == "broker" {
		result, e := readBrokerInput(ctx, os.Stdin)
		err = e
		if wantSuccess && !strings.Contains(result.PEM, "PRIVATE KEY") {
			t.Fatal("successful inherited input was refused")
		}
	} else if fixture[0] == "manual" {
		p := driverProposal()
